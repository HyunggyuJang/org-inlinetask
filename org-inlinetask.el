...
;;
;; Author: Carsten Dominik <carsten at orgmode dot org>, Hyunggyu Jang <murasakipurplez5 at gmail dot com>
...
(defvar org-non-inlinetask-outline-regexp
  (format "\\*\\{1,%s\\} " (1- org-inlinetask-min-level))
  "Regular expression for non-inlinetask-outline.")

(defvar org-non-inlinetask-outline-regexp-bol
  (concat "^" org-non-inlinetask-outline-regexp)
  "Regular expression for non-inlinetask-outline beginning of line.")
...
(defun org-inlinetask-cycle-a (orig-fn &rest args)
  "Resolve corrupted overlaied view by enabling `org-inlintask'.
By injecting `org-outline-regexp-bol'."
  (let ((org-outline-regexp-bol org-non-inlinetask-outline-regexp-bol))
    (apply orig-fn args)))

(defun org-inlinetask-content-a (&optional arg)
  "Fix mangled view of `org-content' when it comes to `org-inlinetask'.
ARG is used to limit the level of headlines to show."
  (interactive "p")
...
(advice-add 'org-cycle :around #'org-inlinetask-cycle-a)
(advice-add 'org-content :override #'org-inlinetask-content-a)
